...
def test_rag_route_only_suggests_evidence_skill(isolated_root):
    init_database()
...
def test_bo_route_blocks_recommendation_when_evidence_is_insufficient(isolated_root):
    ...
    assert "blocked_tools" not in plan.model_dump()
    assert "requires_evidence_gap_check" not in plan.model_dump()
