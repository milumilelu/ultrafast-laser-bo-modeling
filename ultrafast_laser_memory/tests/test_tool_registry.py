...
def test_tool_registry_has_six_foreground_and_three_on_demand_tools():
    registry = build_main_agent_tool_registry()
    assert len(CORE_TOOL_NAMES) == 6
    assert len(ON_DEMAND_TOOL_NAMES) == 3
    assert set(CORE_TOOL_NAMES + ON_DEMAND_TOOL_NAMES) <= {
        item.name for item in registry.list_contracts()
    }
