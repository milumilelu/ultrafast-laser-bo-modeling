...
def test_task_intake_progress_after_chat(isolated_root):
    ...
    assert response.current_stage == "respond"
    assert response.next_required_action["action_type"] == "continue_task"
...
def test_clarification_round_one_progress_percent(isolated_root):
    ...
    assert response.progress["current_stage"] == "respond"
    assert response.progress["progress_percent"] is None
    assert response.next_required_action["action_type"] == "continue_task"
...
def test_llm_failure_keeps_state_without_parser_stall(isolated_root):
    ...
    assert third.workflow_state["current_stage_code"] == "respond"
    assert third.workflow_state["task_spec"]["material"]["name"] == "diamond"
    assert "NextAction" in third.assistant_message
