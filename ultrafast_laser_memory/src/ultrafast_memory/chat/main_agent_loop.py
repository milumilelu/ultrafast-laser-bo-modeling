...
from ultrafast_memory.process_workflow.agent_controller import AgentAction, ProcessAgentController
...
def run_main_agent_turn(
    *, session_id: str, message: str, message_id: str | None, client: Any,
    ...
    controller = ProcessAgentController(client)
    ...
    for turn_step in range(1, max_steps + 1):
        exposed_names = _exposed_tool_names(skills, loaded)
...
def _trace(session_id: str, message_id: str | None, workflow: dict[str, Any],
    ...
    return record_agent_trace_event(
        ...
        status="completed", payload={"action": action.action, "step": step},
    )
