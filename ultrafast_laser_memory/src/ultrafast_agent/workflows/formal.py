...
def complex_process_task() -> WorkflowDefinition:
    return WorkflowDefinition(
        ...
        (
            ...
            WorkflowStep("material_identification", "material_identification", output_key="material_context", skill="material_identification"),
            ...
            WorkflowStep("constraint_extraction", "constraint_extraction", output_key="process_constraints", skill="constraint_extraction"),
            ...
            WorkflowStep("historical_case_retrieval", "similar_case_retrieval", output_key="similar_cases", skill="historical_case_retrieval", parallel_group="context_prefetch"),
            WorkflowStep("process_risk_assessment", "process_risk_assessment", output_key="process_risks", skill="process_risk_assessment"),
            ...
            WorkflowStep("knowledge_use_gate", "knowledge_use_gate", output_key="knowledge_gate_decision", condition=lambda data: bool(data.get("trial_plan")) and bool(data.get("trial_result_validated")), skill="knowledge_use_gate"),
            WorkflowStep("bo_mode_selection", "bo_mode_selection", output_key="bo_status", condition=lambda data: bool(data.get("trial_result_validated")), skill="bo_mode_selection"),
            WorkflowStep(
                "bo_recommendation",
                "bo_recommendation",
                output_key="bo_recommendation",
                condition=lambda data: bool(data.get("trial_result_validated")) and (data.get("knowledge_gate_decision") or {}).get("status") == "allowed",
                timeout_ms=3000,
                skill="bo_recommendation",
                fallback_builder=_bo_fallback,
            ),
            WorkflowStep("quality_plan_generation", "quality_plan_generation", output_key="quality_plan", condition=lambda data: bool(data.get("trial_plan")), skill="quality_plan_generation"),
            WorkflowStep("execution_plan_generation", "execution_plan_generation", output_key="execution_plan", condition=lambda data: bool(data.get("trial_result_validated")), skill="execution_plan_generation"),
            WorkflowStep("report_generation", "report_generation", output_key="task_report", condition=lambda data: bool(data.get("archive_ready")), skill="report_generation"),
        ),
    )
